...
<x-sendportal.text-field name="from_name" :label="__('From Name')" :value="$campaign->from_name ?? old('from_name')" readonly/>
<x-sendportal.select-field name="from_email" :label="__('From Email')" :options="$fromEmails->pluck('from_email', 'from_email')" :value="$campaign->from_email ?? old('from_email')" onchange="" />
...
@push('js')
    <script>
        ...
        $(function () {
            ...
            const names = @json($fromEmails->pluck('from_name','from_email'));

            let service_id = $('select[name="email_service_id"]').val();

            toggleTracking(smtp.includes(parseInt(service_id, 10)));

            let from_email = $('select[name="from_email"]').val();
            $('input[name="from_name"]').val(names[from_email]);

            $('select[name="email_service_id"]').on('change', function () {
              toggleTracking(smtp.includes(parseInt(this.value, 10)));
            });
              
            $('select[name="from_email"]').on('change', function()
            {
                $('input[name="from_name"]').val(names[this.value]);
            });
        });
